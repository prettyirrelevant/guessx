...
export async function prepareMusicContent(
  ...
  const [topData, relatedData] = await Promise.all([
    fetchJson<{ data: Track[] }>(`https://api.deezer.com/artist/${artist}/top?limit=25`),
...
export async function preparePlaceContent(
  ...
  for (const { place, imageUrl } of resolved) {
    ...
    const distractors = shuffle(landmarks.filter((l) => l !== place && !usedNames.has(l))).slice(
      0,
      3,
    );
