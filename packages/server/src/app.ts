...
const createRoomInput = validator("json", (value, context) => {
  if (!isValidCreateRoomInput(value) || !isValidContentConfig(value)) {
    return context.json({ error: "invalid room settings" }, 400);
  }
  return value;
});

const joinRoomInput = validator("json", (value, context) => {
  if (!isValidJoinRoomInput(value)) {
    return context.json({ error: "invalid player profile" }, 400);
  }
  return value;
});

const artistQuery = validator("query", (value) => ({
...
function allowsOrigin(request: Request): boolean {
...
const api = new Hono<HonoEnv>()
  ...
  .use("/rooms/*", requireSession)
  .use("/artists", requireSession)
  .get("/artists", artistQuery, async (context) => {
    const artists = await searchArtistsFromDeezer(context.req.valid("query").query);
    return context.json({ artists });
  })
  .post("/rooms", roomInputBodyLimit, createRoomInput, async (context) => {
  ...
  .post("/rooms/:roomCode/join", roomInputBodyLimit, joinRoomInput, async (context) => {
  ...
  .post("/rooms/:roomCode/preparation", async (context) => {
    const roomCode = context.req.param("roomCode");
    if (!isRoomCode(roomCode)) return context.json({ error: "invalid room code" }, 400);
