import { CONTINENTS, type RoundContent } from "@guessx/game";
...
export async function prepareFlagContent(
  ...
  const pool = COUNTRY_CATALOG.filter((country) =>
    (config.regions as readonly CountryRegion[]).includes(country.region),
  );
